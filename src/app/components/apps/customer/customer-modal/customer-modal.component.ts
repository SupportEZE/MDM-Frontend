...
export class CustomerModalComponent {
  ...
  ngOnInit() {
    this.customerId = this.modalData.customer_id;
    ...
    if (
      this.modalData.pageType === 'document_number' ||
      this.modalData.pageType === 'document_image'
    ) {
    ...
    this.bankForm = this.fb.group({
      ...
      ifsc_code: [
        '',
        [Validators.required, Validators.pattern(/^[A-Z]{4}0[A-Za-z0-9]{6}$/)],
      ],
    ...
    if (this.modalData.pageType === 'assign_distibutor') {
      ...
      if (parentCustomerId) {
        this.distributorForm.controls['parent_customer_id'].setValue(
          parentCustomerId,
        );
        setTimeout(() => {
  ...
  onSearch(search: string, type: any) {
    ...
    if (type === 'customer') {
      this.CommonApiService.getCustomerData(
        '',
        this.modalData.data.login_type_id
          ? this.modalData.data.login_type_id
          : '',
        search,
      );
  ...
  getBeatList(state: string, district: string, search?: string) {
    this.api
      .post(
        { filters: { search: search }, state: state, district: district },
        'beat-route/read-dropdown',
      )
      .subscribe((result) => {
  ...
  onSubmit() {
    if (this.bankForm.valid) {
      ...
      this.api
        .post(this.bankForm.value, 'customer/save-bank-info')
        .subscribe((result) => {
          if (result['statusCode'] == 200) {
  ...
  submitContactPerson() {
    if (this.contactPersonForm.valid) {
      ...
      const functionName = isEditMode
        ? 'customer/update-contact-person-info'
        : 'customer/save-contact-person-info';
      this.api[httpMethod](
        this.contactPersonForm.value,
        functionName,
      ).subscribe((result) => {
  ...
  submitAddress() {
    ...
    if (this.shippingForm.valid) {
      this.shippingForm.value.customer_id =
        this.data.assign_to_id || this.modalData.customer_id;
      if (this.editId) {
      ...
      this.api.disabled = true;
      this.api[httpMethod](this.shippingForm.value, functionName).subscribe(
        (result) => {
          if (result['statusCode'] == 200) {
            this.api.disabled = false;
            this.dialogRef.close(true);
  ...
  addOtherInfo() {
    if (this.otherForm.valid) {
      ...
      this.api.disabled = true;
      this.api
        .patch(this.otherForm.value, 'customer/save-other-info')
        .subscribe((result) => {
          if (result['statusCode'] == 200) {
            this.api.disabled = false;
            this.dialogRef.close(true);
  ...
  updateLocation() {
    if (this.poinLocationForm.valid) {
      ...
      this.api.disabled = true;
      this.api
        .patch(this.poinLocationForm.value, 'customer/save-other-info')
        .subscribe((result) => {
          if (result['statusCode'] == 200) {
            this.api.disabled = false;
            this.dialogRef.close(true);
  ...
  findName(event: any, type: string) {
    ...
    if (Array.isArray(selectedIds) && selectedIds.length > 0) {
      if (type === 'customer') {
        this.distributorForm.value.parent_customer_array =
          this.CommonApiService.customerData.filter((row: any) =>
            selectedIds.includes(row.value),
          );
        return this.distributorForm.value.parent_customer_array;
      }
      if (type === 'user') {
        this.userAssignForm.value.user_array =
          this.CommonApiService.userData.filter((row: any) =>
            selectedIds.includes(row.value),
          );
  ...
  assignUser() {
    if (this.userAssignForm.valid) {
      ...
      this.userAssignForm.value.customer_type_id =
        this.modalData.customer_type_id;
      this.userAssignForm.value.customer_type_name =
        this.modalData.customer_type;
      this.userAssignForm.value.customer_name = this.modalData.customer_name;
      this.api.disabled = true;
      this.api
        .post(
          this.userAssignForm.value,
          'customer/save-user-to-customer-mapping',
        )
        .subscribe((result) => {
          if (result['statusCode'] == 200) {
            this.api.disabled = false;
  ...
  assignDistributor() {
    if (this.distributorForm.valid) {
      ...
      this.distributorForm.value.child_customer_type_name =
        this.modalData.customer_type;
      this.distributorForm.value.child_customer_type_id =
        this.modalData.customer_type_id;
      this.distributorForm.value.child_customer_name =
        this.modalData.customer_name;

      this.api.disabled = true;
      this.api
        .post(this.distributorForm.value, 'customer/assign-customer-mapping')
        .subscribe((result) => {
          if (result['statusCode'] == 200) {
            this.api.disabled = false;
            this.toastr.success(result['message'], '', 'toast-top-right');
  ...
  updateKyc() {
    if (this.kycForm.valid) {
      ...
      this.api.disabled = true;
      this.api
        .patch(this.kycForm.value, 'customer/save-kyc-status')
        .subscribe((result) => {
          if (result['statusCode'] == 200) {
            this.api.disabled = false;
            this.dialogRef.close(true);
  ...
  updateStatus() {
    if (!this.salesSupportForm.value.remark) {
      this.toastr.warning('Remark is required', '', 'toast-top-right');
      return;
    }

    let payload: any = {};

    console.log(this.modalData.pageType, 'modalData.pageType');

    if (this.modalData.pageType === 'Sales Support') {
      payload = {
        ...this.modalData?.checkedStates,
        sales_support: false,
        customer_id: this.modalData.customer_id,
        sales_support_remark: this.salesSupportForm.value.remark,
      };
    }
    if (this.modalData.pageType === 'Basic Detail') {
      payload = {
        customer_id: this.modalData.customer_id,
        ...this.modalData?.checkedStates,
        basic_detail: false,
        basic_detail_remark: this.salesSupportForm.value.remark,
      };
    }
    ...
    this.api
      .post(payload, 'customer/verify-customer-info')
      .subscribe((result) => {
        if (result['statusCode'] == 200) {
          this.api.disabled = false;
          this.dialogRef.close(true);
  ...
  getPostalMaster(pincode: any) {
    this.api
      ...
      .subscribe((result) => {
        if (result['statusCode'] == 200) {
          this.shippingForm.controls['shipping_state'].setValue(
            result['data']['state'],
          );
          this.getDistrict(result['data']['state']);
          this.shippingForm.controls['shipping_district'].setValue(
            result['data']['district'],
          );
          this.shippingForm.controls['shipping_city'].setValue(
            result['data']['city'],
          );
        }
      });
  ...
  onFileProcessed(event: any, type: string) {
    ...
    if (this.modalData.pageType === 'document_image') {
      Object.assign(file, {
        image_type: type,
        doc_no:
          type === 'Pan Card' || type === 'Aadhar Front'
            ? this.updateDocumentForm.value.doc_no
            : '',
      });
    } else {
      Object.assign(file, {
        ...
        doc_no:
          type === 'Pan Card'
            ? this.documentForm.value.pan_no
            : type === 'Aadhar Front'
  ...
  onFileRemove(event: any, type: string) {
    const file = event.file.file;
    if (type === 'Aadhar Front') {
      const index = this.aadharFrontImg.findIndex(
      ...
      const index = this.aadharBackImg.findIndex(
        (f) => f.name === file.name && f.size === file.size,
      );
      if (index > -1) {
      ...
      const index = this.panImg.findIndex(
        (f) => f.name === file.name && f.size === file.size,
      );
      if (index > -1) {
      ...
      const index = this.pondFiles.findIndex(
        (f) => f.name === file.name && f.size === file.size,
      );
      if (index > -1) {
  ...
  updateDocNumber() {
    ...
    this.api
      .patch(this.updateDocumentForm.value, 'customer/update-docs')
      .subscribe((result) => {
        if (result['statusCode'] == 200) {
          this.api.disabled = false;
          this.toastr.success(result['message'], '', 'toast-top-right');
  ...
  updateDocument() {
    if (this.modalData.pageType === 'document_image') {
      ...
      // else {
      this.toastr.error(
        this.data.label + ' Image is required',
        '',
        'toast-top-right',
      );
      // }
  ...
  updatePoint() {
    if (this.pointForm.valid) {
      ...
      this.api.disabled = true;
      this.api
        .post(this.pointForm.value, 'ledger/create-manual')
        .subscribe((result) => {
          if (result['statusCode'] == 200) {
            this.api.disabled = false;
            this.toastr.success(result['message'], '', 'toast-top-right');
